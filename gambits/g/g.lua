...
function refresh_party_status()
    ...
    if (get_pt_party_keys[key]) then
      if (value.mob) then
        ...
        if (party[mob_id]) then
          ...
          if not party[mob_id].rolls then
            party[mob_id].rolls = {}
          end
        end
  ...
  if party[my_id].rolls then -- Handle rolls, whose values we need to track against action packets
    for roll_buff,value in pairs(party[my_id].rolls) do
      if not party[my_id].statuses[roll_buff] then
        party[my_id].rolls[roll_buff] = nil
      end
    end
  end
  ...
  if (action.actor_id == my_id) then
    ...
    elseif ((action.category >= 2) and (action.category <= 5)) then
      if action.category == 2 then
        coroutine.schedule(function()  cta(true) check_gambit_list() end, 2.1)
  ...
  for index,value in pairs(targets) do
    ...
    if (in_party(actor_id) or in_party(target_id) or (enemies[actor_id] ~= nil)) then
      ...
      for _,v1 in pairs(value.actions) do
        ...
        -- User using Phantom Roll
        -- 420 :: <actor> uses <ability>. The total comes to <number>! <target> receives the effect of <ability>.
        -- User using Double-Up
        -- 424 :: <actor> uses Double-Up. The total for <ability> increases to <number>! <target> receives the effect of <ability>.
        -- v1.param is roll value
        if type == 420 or type == 421 then
          if actor_id == my_id then
            if not party[my_id].rolls then
              party[my_id].rolls = {}
            end
            --print(action.param)
            local roll_name = rolls.action_ids[action.param]
            local roll_buff = rolls.values[roll_name].buff_id
            coroutine.schedule(function()
              -- Delay so the check in the buff update packet doesn't immediately nuke our value
              party[my_id].rolls[roll_buff] = v1.param
            end, 0.1)
          end
        end

        -- Party member receiving Phantom Roll OR Double-Up
...
function check_gambit(subject, condition, c_arg, reaction, r_arg)
  ...
  if (reaction == "CHAIN") then
    ...
    if (reaction == "ATTACK") then
      ...
      ability = "ATTACK"
    elseif (reaction == "RANGED_ATTACK") then
      ready = targetting_monster(true)
...
function satisfies_condition(target, condition, c_arg, multiple, cluster)
  ...
  if (target == 'AND') then
    ...
    local switch = {
      ...
      ["CAN_ACT"] = function(target, c_arg) return (can_act()) end,
      ...
      ["SHOULD_DOUBLE"] = function(target, c_arg)
        return (has_status(target.id, 308) and has_status(target.id, c_arg) and rolls.should_double(rolls.values[c_arg], target.rolls[c_arg]))
      end,
...
function react(ability, target)
  ...
  if (ability == "ATTACK") then
    ...
    following_mob = true
  elseif (ability == "RANGED_ATTACK") then
    command = "/ra"
    cast_time_length = 8
...
function process_selector(selector, trigger, trigger_arg, gambit_num)
  local valid_self_triggers = S{"STATUS","NOT_STATUS","NOT_ENGAGED","NOT_ASSISTING","TP >=","HPP <=","HPP >","MPP <=","MA_READY","JA_READY",
    ...
    "SHADOWS <","MOVES <","MOVES >=","NO_PET","PET_HPP <=","PET_IDLE","CAN_SC","CAN_MB","SHOULD_DOUBLE","CAN_ACT"}
  ...
  local triggers_with_raw_args = S{"HPP <=","HPP >","MPP <=","TP >=","NOT_ENGAGED","NOT_ASSISTING","NOT_TAGGED",
      "MANEUVERS <","FIRE_MANEUVERS <","ICE_MANEUVERS <","WIND_MANEUVERS <","EARTH_MANEUVERS <","THUNDER_MANEUVERS <","WATER_MANEUVERS <","LIGHT_MANEUVERS <","DARK_MANEUVERS <",
      "SHADOWS <","MOVES <","MOVES >=","NO_PET","PET_HPP <=","PET_IDLE","CAN_SC","CAN_MB","CAN_ACT"}
  if (triggers_with_raw_args[trigger]) then
    ...
    if (res_table and res_table[trigger_arg]) then
      final_trigger_arg = res_table[trigger_arg].id
    elseif (trigger == "SHOULD_DOUBLE") then
      if rolls.values[trigger_arg] then
        final_trigger_arg = rolls.values[trigger_arg].buff_id
      end
    end
  end
...
function process_reaction(reaction, reaction_arg, gambit_num)
  ...
  if ((reaction == "ATTACK") or (reaction == "ASSIST") or (reaction == "SC") or (reaction == "RANGED_ATTACK")) then
...
if files.exists('my_gambits/'.. my_name ..'.lua') then
  ...
  my_gambits, num_my_gambits = load_gambits('my_gambits')
end
