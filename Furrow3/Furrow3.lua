...
_addon.commands = {'furrow3', 'fw', 'fw3'}
	...
	if running then
		...
		if harvest_furrow_hours[1] == harvest_furrow_hours[2]
		  and harvest_furrow_hours[2] == harvest_furrow_hours[3] then
			windower.add_to_chat(200, 'Furrow3: Harvest times are the same for all seeds, will harvest them after the first sleep cycle.')
			sleepcycle(harvest_furrow_hours[1])
			harvestcycle()
			coroutine.sleep(2)
			running = true
		else
			windower.add_to_chat(200, 'Furrow3: Different harvest times identified, setting harvest order.')
			...
			for _, furrowKey in ipairs(furrowsOrderedByHours) do
				...
				if totalHoursWaited == 0 then
					windower.add_to_chat(200, 'Furrow3: Garden Furrow #'..furrowKey..' is up first. '..harvest_furrow_hours[furrowKey]..'hr growing time.')
					...
					if harvest_furrow_hours[furrowKey] > totalHoursWaited then
						...
						windower.add_to_chat(200, 'Furrow3: Garden Furrow #'..furrowKey..' still has '..adjSleep..'hr(s) remaining.')
						sleepcycle(adjSleep)
			...
			windower.add_to_chat(200, 'Furrow3: Total hours waiting for plants: '..totalHoursWaited)
		end

		-- sell junk items to the moogle if npc_junk is set to true.
		if settings.npc_junk then
			windower.add_to_chat(200, 'Furrow3: Starting the selling junk cycle.')
			selljunkcycle()
			coroutine.sleep(2)
			running = true
		end

		windower.add_to_chat(200, 'Furrow3: Restarting the loop shortly...')
		coroutine.sleep(5)
		loop()
	else
		windower.add_to_chat(200, 'Furrow3: Aborting loop. An operation is already in progress. Please use fw stop or fw abort to cancel current operations.')
	end
end
...
function sleepcycle(hours)
	...
	windower.add_to_chat(200, 'Furrow3: Sleeping for '..hours..' hour(s) before the harvest.')
	...
	for i=1, notices do
		...
		windower.add_to_chat(200, 'Reminder: Furrow3 will commence harvest in '..remainingMin..' minutes. Use fw abort or fw stop to cancel.')
	end
end
...
function target(targetname)
	...
	if windower.ffxi.get_mob_by_target( 't' ) == nil then
		windower.add_to_chat(200, 'Furrow3: No target, cycling.' )
		coroutine.sleep (0.5)
		target(targetname)
	elseif windower.ffxi.get_mob_by_target('t').name == targetname then
		windower.add_to_chat(200, 'Furrow3: Found '..targetname)
	else
		coroutine.sleep(0.5)
		target(targetname)
	end
...
function selljunk()
	windower.add_to_chat(200, 'Furrow3: Selling the junk to the moogle.')
...
function plant(item)
		windower.add_to_chat(200, 'Furrow3: Trading a '..item)
...
function harvest()
		windower.add_to_chat(200, 'Furrow3: Harvesting the furrow.')
		windower.send_command('setkey enter down')
		coroutine.sleep(0.5)
		windower.send_command('setkey enter up')
		coroutine.sleep(5)
...
function harvestcycle(num)
	if running == true then
		if num ~= nil then
			windower.add_to_chat(200, 'Furrow3: Searching for '..furrow_names[num])
			...
			for i = 1, settings.furrows do
				windower.add_to_chat(200, 'Furrow3: Searching for '..furrow_names[i])
		...
		running = false
		windower.add_to_chat(200, 'Furrow3: Harvesting Complete!')
	else
		windower.add_to_chat(200, 'Furrow3: Something went wrong! Please try your command again after reloading Furrow3.')
	end
end

function plantcycle()
	if running == true then
		windower.add_to_chat(200, 'Furrow3: Starting the planting cycle.')
		...
		running = false
		windower.add_to_chat(200, 'Furrow3: Planting Complete!')
	else
		windower.add_to_chat(200, 'Furrow3: Something went wrong! Please try your command again after reloading Furrow3.')
	end
end
...
function furtilizecycle()
	if running == true then
		...
		running = false
		windower.add_to_chat(200, 'Furrow3: Furtilizing Complete!')
	else
		windower.add_to_chat(200, 'Furrow3: Something went wrong! Please try your command again after reloading Furrow3.')
	end
end
...
function selljunkcycle()
	if running == true then
		windower.add_to_chat(200, 'Furrow3: Adding junk to SellNPC queue.')
		...
		windower.add_to_chat(200, 'Furrow3: Searching for Green Thumb Moogle.')
		...
		running = false
		windower.add_to_chat(200, 'Furrow3: Selling junk complete!')
	else
		windower.add_to_chat(200, 'Furrow3: Something went wrong! Please try your command again after reloading Furrow3.')
	end
end
...
function save_settings()
	settings:save()
	windower.add_to_chat(200, 'Furrow3: Settings Saved!')
end

function furrow_command(...)
	if running then
		windower.add_to_chat(167, 'Command cancelled. Furrow3 is currently running. Use fw stop or fw abort to stop the current operation.')
		return
	end
	...
	if #arg > 0 then
		...
		if arg[1]:lower() == 'start' then
			windower.add_to_chat(200, 'Furrow3: Begin loop: Plant > Furtialize > Wait > Harvest > Sell > repeat')
			running = true
			loop()

		elseif arg[1]:lower() == 'stop' or arg[1]:lower() == 'abort' then  -- tested, works
			windower.add_to_chat(200, 'Furrow3: Aborting all actions and reloading.')
			windower.send_command('lua reload furrow3')
...
function showhelp()
	windower.add_to_chat(200, 'Furrow3 commands: start stop abort plant harvest furtilize selljunk.')
end
