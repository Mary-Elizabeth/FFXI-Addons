...
_addon.author = 'Valok@Asura mastered by Nicholas'
...
windower.register_event('addon command', function(...)
	...
	if not target then
		...
		target = windower.ffxi.get_mob_by_name('Ephemeral Moogle')
		if not target or (not self or self.status > 0) then
			print('CrystalTrader: No target selected')
			return
		end
		--if targetM and math.sqrt(targetM.distance) < 6 and targetM.valid_target and targetM.is_npc then
		--		windower.packets.inject_outgoing(0x1A, 'I2H2d2':pack(0xE1A,targetM.id,targetM.index,0,0,0))
		--end
    ...
    packets.inject(packets.new('incoming', 0x058, {
		...
		if target then
			...
			--coroutine.sleep(math.ceil(distance))
		else
			print('CrystalTrader: No target selected')
			return
		end
	...
	if target.name == 'Shami' or target.name == 'Shemo'then
		    ...
		    packets.inject(packets.new('incoming', 0x058, {
				...
				if tradeString ~= '//tradenpc ' then
					if exampleOnly then
						...
						--while(numTrades > 0) -- mary added
							--do -- mary added
								--windower.send_command("wait 0.2; setkey f8 down; wait 0.2; setkey f8 up; wait 0.2") --Mary added
								coroutine.sleep(2)
								windower.send_command('input '..tradeString)
								windower.add_to_chat(8, 'Crystal Trader: '..(numTrades - 1)..' trades remaining')
								--windower.send_command('wait 7') --Mary added
								--numTrades = numTrades-1 -- Mary added
